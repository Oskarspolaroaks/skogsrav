...
const partners = [
  { name: "AVIS", logo: avisLogo },
  { name: "Europcar", logo: europcarLogo },
  { name: "Lynk & Co", logo: lynkCoLogo },
];
  ...
  return (
    <section className="py-16 md:py-20 bg-slate-50">
      <div className="container mx-auto px-6 lg:px-8">
          ...
          {partners.map((partner, index) => (
            ...
            >
              <img
                src={partner.logo}
                alt={partner.name}
                className="h-8 md:h-10 w-auto object-contain grayscale opacity-60 hover:opacity-80 transition-opacity duration-300"
              />
            </motion.div>
          ))}
        </motion.div>
        ...
        >
          Selected references from leasing and mobility partners. Additional client information available upon request.
        </motion.p>
      </div>
    </section>
  );
